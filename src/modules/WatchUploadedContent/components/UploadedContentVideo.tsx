...
const RecordedUploadVideo: React.FC<any> = ({
  videoSrc,
  participantCount = 1,
}) => {
  ...
  return (
    ...
    >
      <Box className="flex w-full h-[284px] gap-2  md:h-[572px]">
            ...
            >
              {participantCount === 1 && (
                <Box
                  ...
                  ref={videoRef}
                  src={videoSrc ?? ""}
                  className="rounded-lg w-full max-h-[280px] min-h-[280px] md:min-h-[572px] object-cover"
                  controls={false}
      ...
      <Box className="flex items-center md:gap-4 gap-2 bg-secondary-main relative -top-1 w-full h-[41px] rounded-br-[12px] rounded-bl-[12px] mx-auto  md:px-4 px-2 ">
        ...
        <Typography
          variant="body2"
          className="text-customGrey-light text-[10px] font-inter-regular font-normal"
        >
          {formatTime(duration)}
        </Typography>
        <Slider
        ...
        <Box
          component={"span"}
          className="text-nowrap text-customGrey-light text-[10px] font-inter-regular font-normal"
        >
        ...
        {volume > 0 ? (
          <VolumeUpIcon
            ...
            className="text-customGrey-thin cursor-pointer"
          />
        ) : (
          <VolumeOffIcon
            onClick={handleVolume}
        ...
        >
          {qualityOptions.map((option) => (
            <MenuItem
              ...
              className="font-inter-regular font-normal"
            >
              {option.label}
            </MenuItem>
          ))}
...
export default RecordedUploadVideo;
